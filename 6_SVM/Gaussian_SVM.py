...
# For Loss = Hinge, Compute the Gaussian kernel matrix:
spread = 1
K = np.zeros([n, n], dtype=np.float32)
for i in range(0, n):
    for j in range(0, n):
        temp = np.linalg.norm(D[i] - D[j])
        K[i][j] = np.exp(-temp**2/(2*spread*spread))
...
for i in range(0, n_test):
    for j in range(0, n_test):
        temp = np.linalg.norm(D_test[i] - D_test[j])
        K_test[i][j] = np.exp(-temp**2/(2*spread*spread))
...
index = []
for i in range(0, n):
    if alpha_gaussian[i] > 0:
        index.append(i)
    else:
        continue

accuracy = count / n_test
print("For Gaussian Kernel, the best combination is:")
print("c = 1, spread = 1")
